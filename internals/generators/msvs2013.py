...
class Msvs2013(BaseGenerator):
    ...
    def GenerateSln(self, _baseSpec, _opts):
        return [
            {
                'outputFilename': "%s.sln" % (_baseSpec['filename']),
                'outputContents': self.generateDotSln(_baseSpec, _opts)
            }
        ]
    ...
    # -----------------------------------------------------------------------------------------------------------------
    # -----------------------------------------------------------------------------------------------------------------
    def generateDotSln(self, _baseSpec, _opts):
        ...
        results = []
        results.append("")
        results.append("Microsoft Visual Studio Solution File, Format Version 12.00")
        results.append("# Generated by antebuild, do not modify by hand. Command Line:")
        results.append("# %s" % " ".join(_opts.argv))
        results.append("#")
        results.append("# Visual Studio 2013")
        results.append("VisualStudioVersion = 12.0.30110.0")
        results.append("MinimumVisualStudioVersion = 10.0.40219.1")

        for prj in _baseSpec['projects']:
            results.append('Project("{%s}") = "%s", "%s.vcxproj", "{%s}"' % (str(uuid.uuid4()).upper(), _baseSpec['name'], prj['filename'], prj['guid'].upper()))
        results.append("EndProject")
        results.append("Global")
        results.append("\tGlobalSection(SolutionConfigurationPlatforms) = preSolution")
        for c in _baseSpec['configurations']:
            for p in _baseSpec['platforms']:
                results.append("\t\t%s|%s = %s|%s" % (c, p, c, p))
        results.append("\tEndGlobalSection")
        results.append("\tGlobalSection(SolutionConfigurationPlatforms) = postSolution")
        for c in _baseSpec['configurations']:
            for p in _baseSpec['platforms']:
                results.append("\t\t{%s}.%s|%s.ActiveCfg = %s|%s" % (_baseSpec['guid'].upper(), c, p, c, p))
                results.append("\t\t{%s}.%s|%s.Build.0 = %s|%s" % (_baseSpec['guid'].upper(), c, p, c, p))
        results.append("\tEndGlobalSection")

        results.append("\tGlobalSection(SolutionProperties) = preSolution")
        results.append("\t\tHideSolutionNode = FALSE")
        results.append("\tEndGlobalSection")
        results.append("EndGlobal")
        results.append("")
